 ...
 * 数组类型克隆器
 */
public class ArrayTypeCloner implements TypeCloner {

    @Override
    public <T> T cloneValue(T t) {
        Log.debug(ArrayTypeCloner.class, "array class:" + t.getClass().getComponentType().getName() + " value:" + t);
    ...
    private void cloneArray(Object sourceArr, Object targetArr){
        ...
        for(int i = 0, len = Array.getLength(sourceArr); i < len; i++){
            ...
            tempObject = Array.get(sourceArr, i);
            if(tempObject.getClass().isArray()){// 如果是多维数组
                value = Array.newInstance(tempObject.getClass().getComponentType(), Array.getLength(tempObject));
                //如果元素是数组，则递归调用
                this.cloneArray(tempObject, value);
            }else{
                // 1. 工厂中能判断包装类别
//                value = TypeClonerFactory.getTypeCloner(tempObject.getClass()).cloneValue(tempObject);
                /**
                 ...
                 */
                // 2. 两种方法都可以实现包装类别判断
                value = TypeClonerFactory.getTypeCloner(sourceArr.getClass().getComponentType()).cloneValue(tempObject);
            }
            // 设置值
            Array.set(targetArr, i, value);
        }
    }
    ...
    public static void main(String[] args) {
         ...
         * 多维数组
         */
        System.out.println(cloner.cloneValue(new int[][]{{1, 2, 3}, {2, 3}}));
    }
     ...
     * @throws Exception
     */
    private static void clone(Object objArr, Object cloneArr) throws Exception {
        Object objTmp;
        Object val = null;
